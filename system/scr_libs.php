...
class ModelJobs {
	...
	public function __construct() {
		$this->db = new db(DB_DRIVER, DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE, DB_PORT, DB_PREFIX);
		$this->config = new ConfigDuble();
		return $this;
	}
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'quantity_two'     => (int)$query->row['quantity_two'],
                                'status_two'       => $query->row['status_two'],
				'stock_status'     => $query->row['stock_status'],
				'image'            => $query->row['image'],
				'manufacturer_id'  => $query->row['manufacturer_id'],
				'manufacturer'     => $query->row['manufacturer'],
				'price'            => ($query->row['discount'] ? $query->row['discount'] : $query->row['price']),
				'price_two'        => $query->row['price_two'],
	...
	public function getSheetProduct($product_id){
    ...
    public function getSheetOrderProduct($order_id){
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` o LEFT JOIN " . DB_PREFIX . "order op ON (o.order_id = op.order_id) LEFT JOIN " . DB_PREFIX . "order_status oz ON (oz.order_status_id = op.order_status_id) WHERE o.order_id = '" . (int)$order_id . "'");
...
class ConfigDuble {
	...
	public function get($key) {
		if($key == 'config_language_id') return 1;
		$query = $this->db->query('SELECT value FROM `oc_setting` WHERE `key`= "'.$key.'" limit 1');
...
class Task {
	...
	public static function errorToFile($file) {
	...
	public static function checkTaskActive() {
	    $db = self::getDB();
